...
import { AdminTabState, TabContext, TabState } from '../../../../state/context/contextState'
...
import MeetupEventLaunch from './meetupEventLaunch'
...
const MeetupGroup = props => {
  const { showMeetupLaunch, closeMeetupEventLaunch } = props.ModalStore
  ...
  if (data) {
    const { name, event } = data.getMeetupGroup
    ...
    return (
      <TabContext.Provider value={TabState}>
        <Header />
        <MeetupEventLaunch
          showMeetupLaunch={showMeetupLaunch}
          closeMeetupEventLaunch={closeMeetupEventLaunch}
          data={data}
          width={Width}
        />
        <br />
        ...
        <Grid>
          ...
          <div style={{ height: window.innerHeight - 100, overflow: 'auto' }}>
            <CSSTransition in={staate.activeTab === 'dashboard'} unmountOnExit timeout={300}>
              <div>
                <Head header style={{ justifyContent: 'center' }}>
                  <div style={{ display: 'flex', justifyContent: 'center' }}>
                    ...
                    <div style={{ margin: '0.3rem 0.7rem', transform: 'rotate(270deg)' }}>
                      <IoIosGitNetwork style={{ fontSize: '1.8rem' }} />{' '}
                    </div>
                    <Title style={{ color: '#0e2f5ac7' }} small center>
                      {event[0].name}
                    </Title>
                  </div>
                </Head>
            ...
            <CSSTransition in={staate.activeTab === 'edit'} unmountOnExit timeout={300}>
              <div>
          ...
          {Width >= 1500 && <Timeline eventData={data.getMeetupGroup.event[0]} />}
        </Grid>
      </TabContext.Provider>
    )
  }
...
export default inject('ModalStore')(observer(MeetupGroup))
