...
import { Hover, Title, Text, Section, Label, Button, Head, Contain } from '../../../styles/style'
...
const Products = props => {
  ...
  const [createCategory, { data, loading, error }] = useMutation(CREATE_CART_CATEGORY)

  const [Category, setCategory] = useState(false)
  const { id, cart_items_category } = props.data.event

  useEffect(() => {
    setcategoryId(cart_items_category === null ? null : cart_items_category[0].id)
  }, ['t'])
  ...
  return (
    <div>
      ...
      <CSSTransition in={ActiveView === 'create'} timeout={300} classNames={''} unmountOnExit>
        <div>
          <Fields
      ...
      <CSSTransition in={ActiveView !== 'create'} timeout={300} classNames={''} unmountOnExit>
        {cart_items_category === null ? (
          <div>
            <Hover style={{ textAlign: 'center' }} onClick={() => setActiveView('create')}>
          ...
          <div>
            ...
            <ProductItem categoryName={categoryName} categoryId={categoryId} screen={'store'} />
          </div>
        )}
      </CSSTransition>
    </div>
